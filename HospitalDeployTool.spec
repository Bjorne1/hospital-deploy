...
# ---------- 只收集必要的 PySide2 DLL ----------
# 项目仅用到 QtWidgets / QtCore / QtGui，排除体积巨大的无用模块
_EXCLUDE_QT_DLLS = re.compile(
    r"Qt5(WebEngine|Quick|Qml|Multimedia|3D|Sensors|Bluetooth|Nfc|"
    r"Positioning|Location|SerialPort|Pdf|RemoteObjects|Sql|Test|"
    r"Designer|Help|Charts|DataVisualization|VirtualKeyboard|"
    r"ShaderTools|SpatialAudio|HttpServer|Grpc|Protobuf|"
    r"LanguageServer|TextToSpeech|Scxml|StateMachine)",
    re.IGNORECASE,
)

pyside2_binaries = [
    (src, dst) for src, dst in collect_dynamic_libs("PySide2")
    if not _EXCLUDE_QT_DLLS.search(Path(src).name)
]
pyside2_binaries += collect_dynamic_libs("shiboken2")

# 只收集 platforms 和 styles 插件（窗口必需），跳过其余插件
import PySide2
_pyside2_dir = Path(PySide2.__file__).parent
_plugins_dir = _pyside2_dir / "plugins"
_NEEDED_PLUGINS = {"platforms", "styles"}
pyside2_datas = []
if _plugins_dir.exists():
    for subdir in _plugins_dir.iterdir():
        if subdir.is_dir() and subdir.name in _NEEDED_PLUGINS:
